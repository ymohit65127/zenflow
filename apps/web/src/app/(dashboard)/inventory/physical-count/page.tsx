...
function CreateCountDialog({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    warehouse_id: "",
    scope: "full" as "full" | "partial" | "cycle",
    notes: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select value={form.scope} onChange={(e) => setForm((f) => ({ ...f, scope: e.target.value as typeof form.scope }))}
              ...
              {(["full", "partial", "cycle"] as const).map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            onClick={() => mutation.mutate({ warehouse_id: form.warehouse_id, scope: form.scope, notes: form.notes || undefined })}
...
export default function PhysicalCountPage() {
  ...
  const completeMutation = api.inventory.physicalCount.complete.useMutation({
    onSuccess: () => { toast.success("Count completed"); void utils.inventory.physicalCount.list.invalidate(); },
    onError: (e) => toast.error(e.message),
  });
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      {["Count #", "Scope", "Lines", "Status", "Created", "Actions"].map((h) => (
                      ...
                      return (
                        <tr key={count.id} className="hover:bg-muted/20">
                          ...
                          <td className="px-5 py-4 text-sm text-muted-foreground">
                            {format(new Date(count.created_at), "MMM dd, yyyy")}
                          </td>
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              {status === "in_progress" && (
                                <button onClick={() => completeMutation.mutate({ id: count.id })} className="flex items-center gap-1 text-xs text-blue-600 hover:underline">
                                  <ClipboardCheck className="w-3 h-3" /> Complete
                                </button>
                              )}
                              {status === "completed" && (
                              ...
                              <Link href={`/inventory/physical-count/${count.id}`} className="flex items-center gap-1 text-xs text-brand-500 hover:underline">
